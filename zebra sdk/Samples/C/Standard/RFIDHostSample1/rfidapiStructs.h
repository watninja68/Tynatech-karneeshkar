...
typedef struct READER_CAPSW
{
	...
	BOOLEAN zoneTransitionSupported;		  //**< reserved1 is used for zoneTransitionSupported that indicates whether it Support Zone transition or not */
...
typedef struct READER_CAPSA
{
	...
	BOOLEAN G2V2CommandSupported; /**< support G2V2 commands such as Authenticate, ReadBuffer, Untraceable, and Crypto */
	BOOLEAN GPSReportingSupported; /**< Can Report GPS Location */
	BOOLEAN zoneTransitionSupported;		  //**< reserved1 is used for zoneTransitionSupported which indicates whether it Support Zone transition or not */
	UINT32  reserved32; /**< Reserved fields for future */
...
typedef struct _TAG_ZONE_INFOW
{
	...
	UINT16 		sourceZoneID;	//**< Unique ID of the source zone from which transition happened. */
	UINT16		tagSpeed;		//**< speed at which tag has moved in foot/sec. */
	LPVOID		lpReserved[3];				/**<  Reserved for future.
											    1 LPVOID is used for sourceZoneID and tagspeed */
...
typedef struct _TAG_ZONE_POINT
{
	INT16	x;	//**< Specify x coordinate of the tag/zone */
	INT16	y;	//**< Specify y coordinate of the tag/zone */
}TAG_ZONE_POINT,*LPTAG_ZONE_POINT;

typedef struct _ZONE_TRANSITION_CONFIG
{
	UINT8	sourceZoneId;	//**< Specifies source zone */
	UINT8	destinationZoneId; //**< Specifies destiantion zone */
	UINT8 	minSpeed;  //**< Specifies minimum speed at which tag can transit in foot/sec */
	UINT8	maxSpeed; //**< Specifies maxmium speed at which tag can transit in foot/sec */
	BOOLEAN	useLHCP;  //**< Specifies whether LHCP is used or not*/
	BOOLEAN useThetaPolarization; //**< Specifies whether ThetaPolarizationLHCP is used or not*/
	BOOLEAN	usePhiPolarization;   //**< Specifies whether PiPolarazation is used or not*/
			
}ZONE_TRANSITION_CONFIG, *LPZONE_TRANSITION_CONFIG;
...
typedef struct _TAG_DATA
{
	...
	TAG_TRANSITION_STATUS tagTransitionStatus;		/**< tagTransitionStatus - True implies tag has transitioned from one zone to another */
	LPTAG_ZONE_POINT  tagCoordinate;	/**< Specify x,y coordinate of the tag */
	LPVOID lpReserved[2]; /**<  Reserved for future.
									1 dword for tagEvent.
									4 dwords for tagEventTimeStamp.
									1 dword for locationInfo
									1 LPVOID used for LPTAG_ZONE_POINT 
									*/
...
typedef struct _ZONE_TRANSITION_READER_CONFIG
{
	UINT16	readerHeight;	//**< Specifies reader height from the floor */
	UINT16	tagHeight;		//**< Specifies tag height from the floor */
	UINT16  zonePlan;		//**< Specifies zone plan of the floor */
	UINT16  zoneInnerWidth;	//**< Specifies inner width of the zone */
	PATH_ALIGNMENT zoneOrientation; //**< Specifies zone orientation of the floor */
	INT16 zoneExtension;
}ZONE_TRANSITION_READER_CONFIG,*LPZONE_TRANSITION_READER_CONFIG;

typedef struct _ZONE_TRANSITION_REPORT_CONFIG
{
	TAG_EVENT_REPORT_TRIGGER zoneEntryEventType;		//**< Report the tag when it enters into the particular zone */
	TAG_EVENT_REPORT_TRIGGER zoneExitEventType;		//**< Report the tag when it exit the particular zone */
	TAG_EVENT_REPORT_TRIGGER zoneTransitionEventType;	//**<Report the tag has transitioned from one zone to the other zone*/
	UINT16  timeout;									//**< Specifies the timeout value to be used for tag Transition event */
}ZONE_TRANSITION_REPORT_CONFIG,*LPZONE_TRANSITION_REPORT_CONFIG;
/**<
